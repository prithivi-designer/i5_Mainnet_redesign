...
export interface TokenUnlockItem {
  ...
  countdownSecondsInitial: number;
  next7dEmission: string;
  next7dPercent: string;
  isWatching: boolean;
...
export interface EarningsReport {
  ...
  impliedMove: string;
  mktCap: string;
  sector: "Technology" | "Consumer" | "Energy" | "Finance" | "Crypto" | "Healthcare";
  isWatching: boolean;
  hasStar?: boolean;
...
const initialUnlockData: TokenUnlockItem[] = [
  ...
  },
  {
    ...
    countdownSecondsInitial: 101564,
  ...
  },
  {
    ...
    countdownSecondsInitial: 239400,
  ...
  },
  {
    ...
    countdownSecondsInitial: 283512,
  ...
  },
  {
    ...
    countdownSecondsInitial: 388800,
  ...
  },
  {
    ...
    countdownSecondsInitial: 436530,
  ...
  },
  {
    ...
    countdownSecondsInitial: 504000,
...
const mockStockReports: EarningsReport[] = [
  {
    id: "aapl",
    ticker: "AAPL",
...
export default function EarningsModal({ isOpen, onClose, initialTab }: EarningsModalProps) {
  ...
  /* ── Stock Earnings States & Filters ────────────────────────── */
  const [stockReports, setStockReports] = useState<EarningsReport[]>(mockStockReports);
  ...
  const [stockWatchlistOnly, setStockWatchlistOnly] = useState<boolean>(false);
  ...
  // Toggle watchlist for Stocks
  const toggleStockWatch = (id: string) => {
  ...
  const isStockDateFiltered =
    selectedYear !== "All years" ||
    selectedMonth !== "All months" ||
    selectedDay !== "All days" ||
    gridSelectedDate !== null;

  // Filtered Crypto Unlocks
  ...
  // Filtered Stock Earnings
  const filteredStockReports = useMemo(() => {
    return stockReports.filter((r) => {
      if (stockSearch.trim()) {
      ...
      if (gridSelectedDate && r.date !== gridSelectedDate) return false;
      if (selectedYear !== "All years" && !r.date.startsWith(selectedYear)) return false;
      ...
      if (stockWatchlistOnly && !r.isWatching) return false;
  ...
  const groupedStockReports = useMemo(() => {
    const map = new Map<string, { label: string; reports: EarningsReport[] }>();
    filteredStockReports.forEach((r) => {
      if (!map.has(r.date)) {
      ...
      map.get(r.date)!.reports.push(r);
    });

    return Array.from(map.entries()).map(([dateKey, value]) => ({
      dateKey,
      dateLabel: value.label,
      countLabel: `${value.reports.length} ${value.reports.length === 1 ? "report" : "reports"}`,
  ...
  const cryptoCalendarDays = useMemo(() => {
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      daysArr.push({ isBlank: false, dayNum: d, dateKey, dayUnlocks });
    }
    return daysArr;
  }, [unlockItems]);

  const stockCalendarDays = useMemo(() => {
    const totalDays = 31;
    const paddingBefore = 3;
    const daysArr = [];
    for (let i = 0; i < paddingBefore; i++) {
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      const dayReports = stockReports.filter((r) => r.date === dateKey);
      daysArr.push({ isBlank: false, dayNum: d, dateKey, dayReports });
    }
    return daysArr;
  }, [stockReports, selectedYear, selectedMonth]);
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        {modalTab === "unlocks" ? (
          ...
          <div className={styles.summaryGrid}>
            <div className={styles.summaryCard}>
              ...
              <div className={styles.insetMainBox}>
                <span className={styles.summaryValue}>{filteredStockReports.length} Reports</span>
              </div>
              <div className={styles.cardFooterRow}>
            ...
            <div className={styles.summaryCard}>
              ...
              <div className={styles.insetMainBox}>
                <span className={styles.summaryValue}>AAPL, AMZN, NVDA</span>
              </div>
              <div className={styles.cardFooterRow}>
                <span className={styles.summarySubtext}>Market movers</span>
                <div className={`${styles.badgePill} ${styles.orangeBadge}`}>
                  <span>●</span> 3 Mega Caps
                </div>
              </div>
            </div>
          </div>
        ...
        {modalTab === "earnings" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            ...
            {/* Dedicated Date & Filters Section */}
            <div className={styles.dateFilterSection}>
              <div className={styles.dateFilterTitleRow}>
                ...
                {isStockDateFiltered && (
                  <button className={styles.resetDateLink} onClick={resetStockDateFilters}>
                    Clear Date Filter ✕
                  </button>
                )}
              </div>
              ...
              <div className={styles.dateControlsRow}>
                {/* Search Bar */}
                <div className={styles.searchInputWrap} style={{ flex: 1, minWidth: 200 }}>
                  <svg width={15} height={15} viewBox="0 0 16 16" fill="none" className={styles.searchIcon}>
                    <circle cx="6.5" cy="6.5" r="4.5" stroke="currentColor" strokeWidth="1.4" />
                    <path d="M10 10l4 4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
                  </svg>
                  <input
                    type="text"
                    placeholder="Search stock ticker, company (e.g. AAPL, NVDA)..."
                    value={stockSearch}
                    onChange={(e) => setStockSearch(e.target.value)}
                    className={styles.searchInput}
                  />
                  {stockSearch && (
                    <button className={styles.clearSearchBtn} onClick={() => setStockSearch("")}>
                      ✕
                    </button>
                  )}
                </div>
                ...
                <div className={styles.fieldGroup}>
                  ...
                  >
                    ...
                    <option value="07">Jul 2026</option>
                    <option value="08">Aug 2026</option>
                    <option value="09">Sep 2026</option>
                  </select>
                </div>
                ...
                {/* Session Select */}
                <div className={styles.fieldGroup}>
                  <label className={styles.fieldLabel}>Session</label>
                  <select
                    className={styles.dateSelect}
                    value={sessionFilter}
                  ...
                  >
                    ...
                    <option value="Pre-Market">Pre-Market (BMO)</option>
              ...
              <div className={styles.filterChipsRow}>
                <button
                  className={`${styles.filterChip} ${
                    highImpliedMove ? styles.activeFilterChip : ""
                  }`}
                  onClick={() => setHighImpliedMove((v) => !v)}
                >
                  ⚡ High Implied Move (&gt;5%)
                </button>
                ...
                <button
                  className={`${styles.filterChip} ${
                    stockWatchlistOnly ? styles.activeFilterChip : ""
                  }`}
                  onClick={() => setStockWatchlistOnly((v) => !v)}
                >
                  ★ Watchlist Only
                </button>
                ...
                {gridSelectedDate && (
                  ...
                  >
                    Selected Day: {gridSelectedDate}
                    <button
                      onClick={() => setGridSelectedDate(null)}
                      style={{
                        background: "none",
                        border: "none",
                        color: "inherit",
            ...
            {/* VIEW 1: AGENDA (CARDS) VIEW (3 CARDS PER ROW) */}
            {viewMode === "agenda" && (
              <div className={styles.agendaContainer}>
                {groupedStockReports.length === 0 ? (
                  <div className={styles.emptyState}>
                    <span className={styles.emptyIcon}>🔍</span>
                    <h4 style={{ color: "var(--text-primary)", margin: 0 }}>
                      No earnings reports match your active filters
                    </h4>
                    <p style={{ color: "var(--text-tertiary)", margin: 0, fontSize: 13 }}>
                      Try clearing your search query or reset date & sector filter selections.
                    </p>
                    <button
                      className={styles.resetFiltersBtn}
                      onClick={() => {
                        resetStockDateFilters();
                        setStockSearch("");
                        setSessionFilter("All sessions");
                        setSectorFilter("All sectors");
                        setStockWatchlistOnly(false);
                          ...
                          <div key={item.id} className={styles.reportCard}>
                            <div className={styles.cardHeader}>
                              <div className={styles.cardHeaderLeft}>
                                ...
                                >
                                  ...
                                  <img
                                    src={
                                      ["NVDA", "MSFT", "TSLA", "AAPL", "AMD", "GOOGL", "AMZN", "META"].includes(item.ticker)
                                        ? `https://icons.duckduckgo.com/ip3/${
                                            ({
                                              ...
                                              AMZN: "amazon.com",
                                              META: "meta.com",
                                            } as Record<string, string>)[item.ticker]
                                          }.ico`
                                        : `https://assets.coincap.io/assets/icons/${item.ticker.toLowerCase()}@2x.png`
                                    }
                                    alt={item.ticker}
                                    style={{
                                      position: "absolute",
                            ...
                            <div className={styles.actionsRow}>
                              <button
                                ...
                                onClick={() => toggleStockWatch(item.id)}
                              >
                                {item.isWatching ? "★ Watching" : "☆ Watch"}
                              </button>
            ...
            {/* VIEW 2: CALENDAR GRID VIEW FOR STOCKS */}
            {viewMode === "calendar" && (
              <div className={styles.calendarGridWrapper}>
                <div className={styles.calendarGridHeader}>
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((dayName) => (
                ...
                <div className={styles.calendarDaysGrid}>
                  {stockCalendarDays.map((cell, idx) => {
                    if (cell.isBlank) {
                      return <div key={`blank-${idx}`} className={styles.blankDayCell} />;
                    }

                    const hasReports = (cell.dayReports?.length || 0) > 0;
                    ...
                    return (
                      <div
                        key={cell.dateKey}
                        className={`${styles.calendarDayCell} ${
                          hasReports ? styles.hasReportsDayCell : ""
                      ...
                      >
                        <div className={styles.cellDayNumberRow}>
                          ...
                          {hasReports && (
                            <span className={styles.cellReportCount}>
                              {cell.dayReports?.length} earnings
                            </span>
                          )}
                        </div>
                          ...
                          {cell.dayReports?.map((r) => (
                            <div
                              ...
                              style={{
                                ...
                                color: r.session === "AH" ? "#a5b4fc" : "#fbbf24",
                              }}
                            >
                              <span className={styles.miniTickerSymbol}>{r.ticker}</span>
                              <span className={styles.miniTickerMove}>{r.impliedMove}</span>
                            </div>
                          ))}
                        </div>
                      </div>
